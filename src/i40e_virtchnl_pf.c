					  ...
					  u16 rule_type, u16 rule_id)
{
	...
	struct i40e_pf *pf = src_vsi->back;
	int ret;
	printk("entered del_ingress_egress\n");
...
#else
#error HAVE_NDO_SET_VF_LINK_STATE not defined
 ...
 * Returns 0 on success, negative on failure
 **/
int i40e_set_ingress_mirror(struct net_device *netdev, int vf_id,
				   const int mirror)
{
	struct i40e_netdev_priv *np = netdev_priv(netdev);
	struct i40e_vsi *vsi = np->vsi;
	...
	u16 rule_type, rule_id;
	int ret;
	printk("Checkpoint1 vf_mirror_vf ingress\n");
	...
	printk("Checkpoint2 vf_mirror_vf ingress\n");
err_out:
	return ret;
}
 ...
 * Returns 0 on success, negative on failure
 **/
int i40e_get_ingress_mirror(struct net_device *netdev, int vf_id, int *mirror)
{
	struct i40e_netdev_priv *np = netdev_priv(netdev);
	struct i40e_vsi *vsi = np->vsi;
	struct i40e_pf *pf = vsi->back;
 ...
 * Returns 0 on success, negative on failure
 **/
int i40e_get_egress_mirror(struct net_device *netdev, int vf_id, int *mirror)
{
	struct i40e_netdev_priv *np = netdev_priv(netdev);
	struct i40e_vsi *vsi = np->vsi;
	struct i40e_pf *pf = vsi->back;
 ...
 * Returns 0 on success, negative on failure
 **/
int i40e_set_egress_mirror(struct net_device *netdev, int vf_id,
				  const int mirror)
{
	struct i40e_netdev_priv *np = netdev_priv(netdev);
	struct i40e_vsi *vsi = np->vsi;
 ...
 * Returns 0 on success, negative on failure
 **/
int i40e_get_pf_ingress_mirror(struct net_device *netdev, int *mirror)
{
	struct i40e_netdev_priv *np = netdev_priv(netdev);
	struct i40e_vsi *vsi = np->vsi;
	struct i40e_pf *pf = vsi->back;
 ...
 * Returns 0 on success, negative on failure
 **/
int i40e_set_pf_ingress_mirror(struct net_device *netdev, const int mirror)
{
	struct i40e_netdev_priv *np = netdev_priv(netdev);
	struct i40e_vsi *vsi = np->vsi;
	struct i40e_pf *pf = vsi->back;
 ...
 * Returns 0 on success, negative on failure
 **/
int i40e_get_pf_egress_mirror(struct net_device *netdev, int *mirror)
{
	struct i40e_netdev_priv *np = netdev_priv(netdev);
	struct i40e_vsi *vsi = np->vsi;
	struct i40e_pf *pf = vsi->back;
 ...
 * Returns 0 on success, negative on failure
 **/
int i40e_set_pf_egress_mirror(struct net_device *netdev, const int mirror)
{
	struct i40e_netdev_priv *np = netdev_priv(netdev);
	struct i40e_vsi *vsi = np->vsi;
	struct i40e_pf *pf = vsi->back;
...
int i40e_set_both_mirror(struct net_device *netdev, int vf_id,
						  const int mirror)
{
	int ret;
	printk("setting ingress mirror\n");
	ret = i40e_set_ingress_mirror(netdev,vf_id,mirror);
	if (ret)
		goto err_out;

	printk("setting egress mirror\n");
	ret = i40e_set_egress_mirror(netdev,vf_id,mirror);
	if (ret)
		goto err_out;
 ...
 * negative on failure
 **/
int i40e_get_mirror(struct pci_dev *pdev, int vf_id,
...
int i40e_rem_mirror(struct net_device *netdev, int vf_id,
				  int mirror){
	int ret;
	printk("removing ingress mirror");
	ret=  i40e_set_ingress_mirror(netdev,vf_id,mirror);
	if (ret)
		goto err_out;

	printk("removing egress mirror\n");
	ret = i40e_set_egress_mirror(netdev,vf_id,mirror);
	if (ret)
		goto err_out;
